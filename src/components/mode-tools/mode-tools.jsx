...
class FontAwesomeSearchPanel extends React.Component {
    ...
    _toggle (icon) {
        const { addedIds } = this.state;
        const next = new Set(addedIds);
        if (next.has(icon.id)) {
            removeFontAwesomeShape(icon.id);
            next.delete(icon.id);
            if (this.props.onRemoveIcon) this.props.onRemoveIcon(icon.id);
        } else {
...
const ModeToolsComponent = props => {
            ...
            case Modes.SUSSY:
    {
        ...
        class SussyShapeList extends React.Component {
            ...
            render () {
                const shapes = sussyToolShapes();
                const categorized = categorizeSussyShapes(shapes);
                return (
                    <div>
                        <InputGroup className={classNames(styles.modDashedBorder, styles.dropItemShapeToolMenu, styles.dropdownMaxItemList)}>
                            {Object.keys(categorized).map(categoryId => categorized[categoryId].length === 0 ?
                                (<React.Fragment key={categoryId} />) : (<React.Fragment key={categoryId}>
                                    <p className={classNames(styles.dropItemShapeToolLabel)}>{sussyToolCategories[categoryId]}</p>
                                    {categorized[categoryId].map(shape => (
                                        <LabeledIconButton
                                            key={shape.id}
